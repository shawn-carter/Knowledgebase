...
class NonAuthenticatedUserRedirectTest(TestCase):
    ...
    def test_article_detail_redirect(self):
        ...
        self.assertRedirects(
            response, "/login/?next=" + reverse("article_detail", args=[1])
        )

    def test_edit_article_redirect(self):
        response = self.client.get(reverse("edit_article", args=[1]))
        self.assertRedirects(
            response, "/login/?next=" + reverse("edit_article", args=[1])
        )
...
class PasswordResetRequestViewTestCase(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="user", email=self.email, password="password"
        )
    ...
    def test_password_reset_for_existing_user(self):
        response = self.client.post(
            reverse("password_reset_request"), data={"email": self.email}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_password_reset_for_non_existing_user(self):
        response = self.client.post(
            reverse("password_reset_request"), data={"email": "nonexistent@example.com"}
        )
        self.assertEqual(response.status_code, 200)
...
class SuperUserViewsTest(BaseTestCaseWithSuperUser):
    def test_superuser_urls(self):
        ...
        urls = [
            ...
            reverse("user-list"),
            reverse(
                "toggle_user_active_status", args=[1]
            ),  # replace 1 with actual user_id
            reverse(
                "delete_article", args=[self.article.id]
            ),  # replace 1 with actual article_id
            reverse(
                "undelete_article", args=[self.article.id]
            ),  # replace 1 with actual article_id
