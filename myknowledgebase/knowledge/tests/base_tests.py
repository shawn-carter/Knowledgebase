...
class BaseTestCaseWithSuperUser(BaseTestCaseWithUser):
    ...
    def check_url_for_different_user_types(self, url):
        ...
        if url in redirect_pages_for_superuser:
            try:
                self.assertEqual(response.status_code, 302)  # Expecting a redirect after performing an action
            except AssertionError:
                print(f"AssertionError for URL: {url}")
                raise
        else:
            try:
                self.assertEqual(response.status_code, 200)  # Expecting a successful response, since this is a rendered page
            except AssertionError:
                print(f"AssertionError for URL: {url}")
                raise
